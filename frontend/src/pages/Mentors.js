    ...
    return(
        <div
            ...
            <div class="rounded-t-lg h-32 overflow-hidden">
                <img class=" object-cover w-full" src={coverPic?.secure_url ?? DEFAULT_COVER_IMG} alt={`${username} coverpic`} />
            </div>
            <div class="mx-auto w-32 h-32 relative -mt-16 border-4 border-white rounded-full overflow-hidden">
                <img class="object-cover object-center h-32" src={profilePic?.secure_url ?? DEFAULT_USER_IMG} alt={`${username} profilepic`} />
            </div>
            <div class="text-center mt-2">
                <Link to={"/mentee/" + userID + "/profile"} class="h2 font-semibold dark:text-slate-300">{username}</Link>
                <p class="text-gray-500">{jobDescription ? jobDescription : null}</p>
            </div>
  ...
  return (
		<div className="dark:bg-gray-800 pt-4">
			...
			<div className="grid grid-cols-1 lg:grid-cols-4 lg:gap-5 lg:px-4 ">
				{
					mentorsResult?.map(mentorDet => <Card key={mentorDet?._id} details={mentorDet} /> )
				}
